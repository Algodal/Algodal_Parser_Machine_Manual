...
| `to_number` | Converts the parsed result to a number, 0 to max (negative and decimal numbers not supported) |
| `part` | Returns a sub-text of the parsed text |
| `count` | Returns the iterator count of a counter (how many times it repeated) |
...
t5 = <A:Z>+::count; # number of characters matched by the counter
t5b = (A B C)-5:10::count; # return the number of times the series A B C was parsed
```